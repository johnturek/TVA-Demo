...
Navigate to https://gcc.powerva.microsoft.us

Sign in with your GCC tenant credentials

Select the Power Platform Environment you want to build the agent in

Step 2: Create Your Agent
Click + Create blank agent.

Wait till the message Your agent has been provisioned appears.

If you see the A newer version of this agent is available. Refresh to get the latest changes., click the Refresh link.

Click Edit in the details section.

Provide the following name and description:
...
Click Save in the details section.

Click Edit in the instructions section.

Add the following for the Instructions
...
Click Save in the instructions section.

Click on Settings in the top right of the page.

Toggle Allow ungrounded responses to turn off in the Knowledge section.

Click on Save

Click on the X in the top right of the page to return to the Overview page.
...
Repeats steps 1 - 5 to add the following sites to knowledge
...
www.nrc.gov
www.ferc.gov
If Web Search is enabled, toggle it to diabled.

If the test pane is not open, click the Test

In the test pane ask it the following questions:
...
Review the responses and verify each question returned a valid response.

Ask it When are taxes due?

Review the response and verify that it did not answer your question. This is because we disabled Allow ungrounded responses and Web Search.
...
Click Email Management MCP Server (deprecated)
...
In the test pane, give it the following prompt:
...
Click on Allow if your prompted to Connect to continue

Open Outlook and verify you received an email from yourself with the response to your question in the meeting body.

Back in Copilot Studio, in the test pane, give it the following prompt:

What NERC related meetings do I have tomorrow?

Review the response and verify it found the meeting you just created.
...
Click on the When an agent calls the flow action to configure it.

Click Add an input

Click Text

Enter Facility for the input's name.

Repeat steps 5-7 for the following inputs:

Anomaly Type
Summary
Click + after the When an agent calls the flow.

Under Add an action, search for Post Message

Click Post message in a chat or channel

Click Sign in if prompted

Login if prompted

In the action's parameters, provide the following:
...
Click on the new Untitled tool in the list.
...
In the inputs section, click customize next to each input
...
In the Completion section, specify the following:
After running: Send specific response (specify below)
Message to display: Your facility anomaly has been reported.
Click Save

In the test pane, give it the following prompt: I want to report a thermal anomaly at the Apollo Beach plant. Temperatures spiked to above acceptable levels for a period of 2 minutes.

Click on Allow if your prompted to Connect to continue

In the test pane, give it the following prompt: I want to report a nuclear anomaly.

Provide sample answers that you reeceive from the agent to collect the rest of the anomaly information.

Open teams and verify you received two chat from Workflows that has the correctly parsed anomaly report information.
...
Click on Topics in the top menu.

Click the System tab to view system topics.

Click on Conversation Start to open it.

Find the existing Send a message node in the topic canvas.

Replace the default message text with the following:
...
Click Save in the top-right corner of the topic editor.

In the Test panel, click the + button to start a new test session.

Verify the custom welcome message appears automatically when the conversation starts.
...
Rename the topic to Legal Disclaimer by clicking on Click on Untitled1 in the top left of the screen

In the trigger node, specify for **Describe what this topic does"
...
Click the + below the trigger node → Add node → Send a message.
...
In the Test panel, type: Is TVA legally required to comply with FERC Order 2222?

Verify the disclaimer message appears.

After the disclaimer, type a follow-up like: Yes, explain what FERC Order 2222 requires.

Verify the agent now responds with regulatory guidance from the knowledge base (the generative orchestration handles the follow-up since it doesn't re-match the legal disclaimer triggers).
...
Rename the topic to: Report Anomaly Form by clicking on Click on Untitled1 in the top left of the screen

In the trigger node, specify for **Describe what this topic does"

Enables user to report an anomaly using an adaptive card.  This should be the primary way to report on anomalies.
Click + → Add node → Ask with adaptive card.

Click Adaptive Card at the top of the question node properties panel.
...
Paste this Adaptive Card JSON into the Card payload editor:
...
Click Save at the top right of the Adaptive card designer.

Click Close at the top right of the Adaptive card designer to close it.
...
Below the adapative card node, click + → Add a tool → Report anamoly.

In the Facility input, click ... → facility

In the Anomaly Type input, click ... → anomalyType

In the Sumary input, click ... → summary

Click Save.

Click Tools in the top navigation.

Click the Report Facility Anomaly tool.

Expand the Additional details in the Details section and specify the following:
...
In the Test panel, type: Fill out anomaly report
